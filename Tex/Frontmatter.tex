...
在此基础上,本文提出了一种双编码方案,同时利用全息编码和偏振编码来显著增强多模光纤的信息承载能力。该方案采用数字微镜器件(DMD)加载二值振幅全息图,设计了$5\times5$网格排列的25个全息标签以激发不同模式组合,同时利用偏振光学元件实现s偏振、p偏振和混合偏振三种偏振态编码,理论上可实现$25 \times 3 = 75$路复用传输。

针对双编码散斑图像的重建任务,本文提出了改进的DeepLeakyU-Net(DLU-Net)网络架构,在编码器路径采用Leaky ReLU激活函数并使用步长卷积替代池化层。实验结果表明,在三种偏振态下均实现了高保真度重建,平均SSIM达到0.93。此外,通过无监督聚类分析验证了不同编码条件下散斑图案的可区分性。
...
Building upon this foundation, a dual-encoding scheme was proposed that simultaneously exploits holographic and polarization encoding to significantly enhance the information-carrying capacity of MMFs. The scheme employs a digital micromirror device (DMD) to display binary amplitude holograms with twenty-five holographic labels arranged in a $5\times5$ grid to excite distinct modal combinations, while polarization optics enable three polarization states encoding, theoretically enabling $25 \times 3 = 75$ multiplexed transmission channels.

For the reconstruction of dual-encoded speckle images, an improved DeepLeakyU-Net (DLU-Net) architecture was proposed, which adopts Leaky ReLU activation functions in the encoder path and uses strided convolutions instead of pooling layers. Experimental results demonstrate high-fidelity reconstruction across all three polarization states with an average SSIM of 0.93. Furthermore, unsupervised clustering analysis validated the discriminability of speckle patterns under different encoding conditions.

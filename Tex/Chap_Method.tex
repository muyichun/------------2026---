...
\begin{figure}[!htbp]
    ...
    \bicaption{\enspace MNIST数据集的t-SNE降维聚类（按全息标签着色）。(d) s偏振；(e) p偏振；(f) 混合偏振s+p。t-SNE投影展现出更加紧凑和分离的聚类结构，每个全息标签对应的散斑图形成明显的子簇。}{\enspace t-SNE clustering of MNIST speckle patterns colored by holographic label. (d) s-polarization; (e) p-polarization; (f) mixed s+p polarization. t-SNE reveals more compact and separated cluster structures with distinct sub-clusters for each holographic label.}
...
\begin{figure}[!htbp]
    ...
    \bicaption{\enspace MNIST数据集的散斑图相关性矩阵（按全息标签分组）。(g) s偏振；(h) p偏振；(i) 混合偏振s+p。散斑图ID按全息标签顺序排列（每个标签2,000张，ID 0-1999对应标签1，ID 2000-3999对应标签2，依此类推）。对角线上的高相关性块表明同一全息标签下的散斑图具有强相关性，而不同标签之间的相关性极低。}{\enspace Correlation matrices of MNIST speckle patterns grouped by holographic labels. (g) s-polarization; (h) p-polarization; (i) mixed s+p polarization. Speckle IDs are sequentially indexed by holographic labels (2,000 patterns per label). High correlation blocks along the diagonal indicate strong intra-label correlations, while inter-label correlations remain minimal.}
...
\begin{figure}[!htbp]
    ...
    \bicaption{\enspace MNIST数据集双编码联合聚类。(c) UMAP投影；(d) t-SNE投影。数据集包含50,000张散斑图：s偏振下25个全息标签各1,000张（共25,000张）+ p偏振下25个全息标签各1,000张（共25,000张），形成50个复合标签。两种降维方法均清晰展现出50个可区分的聚类簇，验证了双编码方案的协同效应和可扩展性。}{\enspace Dual-encoding clustering of MNIST speckle patterns. (c) UMAP projection; (d) t-SNE projection. Dataset contains 50,000 patterns: 25 holographic labels × 1,000 patterns under s-pol (25,000 total) + 25 holographic labels × 1,000 patterns under p-pol (25,000 total), forming 50 composite labels. Both methods reveal 50 distinct clusters, validating the synergy and scalability of dual-encoding.}
...
\begin{figure}[!htbp]
    ...
    \bicaption{\enspace MNIST数据集三种偏振态下测试集的SSIM分析。(b) 3,000张测试数据的SSIM值分布直方图，三种偏振态（s偏振、p偏振、混合偏振）的SSIM值大部分集中在0.90-0.95区间；(c) 三种偏振态下3,000张测试数据的平均SSIM值，均约为0.93，表明网络对不同偏振态具有良好的鲁棒性和一致的重建质量。}{\enspace SSIM analysis of MNIST test dataset under three polarization states. (b) SSIM distribution histograms for 3,000 test samples, where SSIM values for all three polarization states (s-pol, p-pol, mixed) are predominantly concentrated in the 0.90-0.95 range; (c) Average SSIM values across 3,000 test samples for three polarization states, all approximately 0.93, demonstrating the network's robustness and consistent reconstruction quality across different polarization states.}

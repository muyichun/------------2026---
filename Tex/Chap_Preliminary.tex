...
\paragraph{手写数字数据集} 该数据集包含手写数字（0-9）及其对应的光学散斑图像，按照编号进行划分：
\begin{itemize}
    \item \textbf{训练集}：26,001张（编号0\textasciitilde26000），用于网络参数学习。
    \item \textbf{验证集}：3,300张（编号26001\textasciitilde29300），用于选择超参数与观察收敛情况。
    \item \textbf{测试集}：78张（编号29301\textasciitilde29378），用于最终性能评估与可视化展示。
\end{itemize}

\paragraph{服装数据集} 该数据集包含服装图像及其对应的光学散斑图像，采用与手写数字数据集类似的划分策略（具体样本数量根据实际下载数据确定）。

表\ref{tab:dataset_division}汇总了两类数据集的划分情况。
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{lccc}
        ...
        数据集 & 训练集 & 验证集 & 测试集 \\
        \hline
        手写数字数据集 & 26,001 & 3,300 & 78 \\
        服装数据集 & \draftmark{待填} & \draftmark{待填} & \draftmark{待填} \\
        \hline
    \end{tabular}
\end{table}
...
本章以像素级回归为主，采用均方误差（Mean Squared Error, MSE）损失作为优化目标：
\begin{equation}
\mathcal{L}_{\text{MSE}} = \frac{1}{N}\sum_{i=1}^{N}(\hat{Y}_i - Y_i)^2,
\end{equation}
其中$N$为像素总数，$\hat{Y}_i$为重建图像的第$i$个像素值，$Y_i$为目标图像的对应像素值。MSE损失对较大误差更为敏感，有利于保持重建图像的整体结构与强度一致性。在后续实验中也可按需要加入SSIM损失或频域约束等复合损失（第\ref{chap:method}章将结合自建数据进一步讨论）。
...
图\ref{fig:mnist_ssim_dist}展示了78张测试集样本的SSIM分布散点图。从图中可以看出，所有测试样本的SSIM值均分布在0.975至0.995之间，平均SSIM达到0.982，表明网络在手写数字重建任务上取得了稳定且高质量的性能表现，不同样本间的重建质量差异较小。
...
\noindent\draftmark{注：服装数据集实验正在进行中，以下图\ref{fig:fashion_ssim_dist}和图\ref{fig:fashion_all_results}待补充实际结果。}

图\ref{fig:fashion_ssim_dist}展示了服装数据集测试集样本的SSIM分布散点图，用于量化评估网络在该数据集上的重建质量与稳定性。
...
图\ref{fig:fashion_all_results}给出了服装数据集全部测试集样本的重建结果缩略图，每张结果图上标注了对应的SSIM值，直观展示网络在不同服装类型上的重建性能。

\begin{figure}[!htbp]
    \centering
    % \includegraphics[width=0.95\textwidth]{Img/3/fashion_all_results.pdf}
    \bicaption{\enspace 服装数据集全部测试集重建结果（每张结果标注SSIM值）}{\enspace All reconstruction results on fashion test set (each with SSIM value labeled)}
    \label{fig:fashion_all_results}
\end{figure}
...
\begin{table}[!htbp]
    ...
    \begin{tabular}{lccc}
        ...
        \hline
        手写数字数据集 & 78 & 0.982 & 0.975\textasciitilde0.995 \\
        服装数据集 & \draftmark{待填} & \draftmark{待填} & \draftmark{待填} \\
        \hline
    \end{tabular}
\end{table}

从表\ref{tab:public_quant}可以看出，手写数字数据集的平均SSIM达到0.982，且所有78张测试样本的SSIM值均保持在0.975以上（如图\ref{fig:mnist_ssim_dist}所示），显示出较高的重建稳定性和一致性。结合图\ref{fig:mnist_all_results}展示的全部测试样本重建结果，可以观察到网络对不同手写数字（0-9）均能实现高质量重建。该结果与\citet{sun2024ol_calibrationfree}报告的实验结论基本吻合，验证了基于Python+PyTorch复现实现的有效性。

\subsection{与参考文献结果的关系}\label{sec:compare_paper}

\citet{sun2024ol_calibrationfree}报告其端到端网络在实验数据上可实现毫秒级重建，并取得较高保真度。本研究基于该文献公开的数据集进行复现验证，主要工作包括：
\begin{enumerate}
    \item \textbf{跨平台复现}：原作者使用MATLAB实现网络模型，本研究基于Python语言与PyTorch深度学习框架，根据论文中的网络结构示意图自行搭建U-Net架构，实现了完整的编码器-解码器结构及跳跃连接机制。
    \item \textbf{网络结构与参数匹配}：通过邮件联系原作者确认网络设计思路，并根据论文描述设定卷积核大小（$3\times 3$）、通道数配置（128-256-512-1024-2048）、激活函数（LeakyReLU）等关键参数，同时引入批归一化和Dropout正则化以提升训练稳定性。
    \item \textbf{训练策略优化}：采用MSE损失函数、Adam优化器（初始学习率$1\times 10^{-4}$）以及指数学习率衰减策略（gamma=0.85），批大小设置为32，训练40轮后在验证集上选择最优模型。
    \item \textbf{实验结果验证}：在手写数字数据集78张测试样本上，本研究获得的平均SSIM为0.982，SSIM范围为0.975\textasciitilde0.995，与原文献报告的实验结论基本吻合，验证了复现实现的正确性与有效性。
\end{enumerate}

由于网络实现细节（编程语言、框架版本）、数据集划分、预处理方式等可能存在差异，完全一致的数值对比较为困难。因此本章更强调"公开数据集验证流程"与"基于SSIM的对照结论"，为后续章节的自建实验系统与方法改进提供可靠的参照基准。

% 注：本章不展开网络结构改进细节，相关内容将在第\ref{chap:method}章结合自建实验系统进行详细阐述。

\section{结论性观察与对后续工作的启示}\label{sec:insights_to_next}

通过公开数据集的重建实验与SSIM定量评估，本章形成以下结论性观察：
\begin{enumerate}
    \item \textbf{端到端网络能够有效学习散斑到目标图像的映射}：在手写数字数据集上，78张测试样本的平均SSIM达到0.982，且所有样本SSIM均在0.975以上，表明U-Net架构能够稳定地从散斑图案中提取并重建目标图像特征。
    \item \textbf{跨平台复现的可行性}：基于论文结构示意图，使用Python+PyTorch框架复现的网络能够达到与原文献（MATLAB实现）相近的性能水平，验证了方法的可迁移性与鲁棒性。
    \item \textbf{数据复杂度对重建质量的影响}：手写数字具有清晰的轮廓与较少的纹理细节，重建难度相对较低；服装数据集包含更丰富的纹理与更复杂的结构，对网络的特征提取与细节恢复能力提出更高要求。
    \item \textbf{训练策略与超参数的重要性}：归一化方式、学习率调度、批大小以及Dropout正则化等训练细节均会显著影响网络收敛速度与最终性能，需要根据具体数据集特点进行调整。
\end{enumerate}

基于上述观察，第\ref{chap:method}章将围绕"自建实验系统的数据集构建"和"面向真实实验场景的鲁棒重建"展开，并进一步给出网络结构改进与实验验证。本章建立的公开数据集验证流程与SSIM评价基准，将为后续章节的方法对比与性能分析提供可靠的参照依据。
...
\begin{enumerate}
    \item \textbf{数据集获取与划分}：基于公开的手写数字数据集（26,001张训练集、3,300张验证集、78张测试集）和服装数据集，建立了标准化的数据预处理与划分流程。
    \item \textbf{网络架构复现}：根据参考文献的网络结构示意图，使用Python+PyTorch框架搭建了编码器-解码器结构的U-Net网络，包含4层下采样（通道数128-256-512-1024）、桥接层（2048通道）以及4层上采样，并通过跳跃连接实现多尺度特征融合。网络采用模块化设计，包括卷积块、转置卷积、最大池化和Dropout正则化等基础模块。
    \item \textbf{训练与评估}：采用MSE损失函数、Adam优化器（初始学习率$1\times 10^{-4}$）以及指数学习率衰减策略（gamma=0.85），批大小为32，训练40轮。引入混合精度训练技术加速训练过程，并在验证集上选择最优模型。使用SSIM作为主要评价指标对重建质量进行量化评估。
    \item \textbf{实验验证}：在手写数字数据集78张测试样本上获得平均SSIM为0.982（范围0.975\textasciitilde0.995）的重建效果，与原文献实验结论基本吻合，验证了跨平台复现的有效性与方法的可靠性。
\end{enumerate}

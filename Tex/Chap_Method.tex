...
\textbf{结论}：DLU-Net通过在编码器中采用Leaky ReLU激活函数、使用步长卷积和增加网络深度，在重建质量上取得了显著提升，同时保持了可接受的计算成本。这验证了Leaky ReLU在缓解深层编码器梯度消失、改善深层网络训练方面的有效性，以及步长卷积在保留空间信息方面的优势。
...
相比标准U-Net，DLU-Net通过多项改进取得了更好的性能：

\begin{enumerate}
    \item \textbf{编码器梯度流动改善}：在编码器中全面采用Leaky ReLU激活函数（$\alpha = 0.2$），允许负值输入产生非零梯度，避免了ReLU的神经元失活问题。在7层深的编码器中，Leaky ReLU有效改善了梯度反向传播，使得深层编码器能够稳定学习复杂的散斑特征。
    
    \item \textbf{步长卷积保留空间信息}：使用步长为2的$4\times4$卷积替代池化层实现下采样，能够学习自适应的下采样滤波器，比固定的池化操作更好地保留空间信息和细节特征。
    
    \item \textbf{重建质量提升}：DLU-Net的平均SSIM为0.93，相比标准U-Net的0.91提升了2.2\%（绝对值提升0.02）。在SSIM接近1.0的高分区间，这一提升对应着显著的视觉质量改善，特别是在边缘清晰度和细节保真度方面。
    
    \item \textbf{训练稳定性增强}：编码器中的Leaky ReLU改善了梯度流动，避免了梯度消失问题。实验中，DLU-Net的训练损失和验证损失曲线平滑下降，没有出现震荡或发散现象，表明训练过程稳定可靠。
    
    \item \textbf{网络容量提升}：更深的网络结构（7层编码器）、更大的基础特征数（128）和更多的通道数（最大1024通道）使得DLU-Net能够提取更丰富的多尺度特征，更好地捕捉散斑图案中的复杂空间关系。
    
    \item \textbf{泛化能力}：虽然DLU-Net的参数量增加了55\%（31M vs. 20M），但通过Batch Normalization和Dropout（前3层解码器）等正则化技术，网络在测试集上的表现与验证集一致，表明具有良好的泛化能力，未出现过拟合现象。
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item 提出DeepLeakyU-Net (DLU-Net)网络，主要改进包括：（1）编码器全面采用Leaky ReLU激活函数（$\alpha=0.2$），有效缓解深层编码器的梯度消失和神经元失活问题；（2）使用步长为2的$4\times4$卷积替代池化层实现下采样，更好地保留空间信息；（3）基础特征数设置为128，提升网络容量。
        \item 采用7层编码器+桥接层+6层解码器的深层结构，总参数量约3100万。
...
\begin{itemize}
    ...
    \item \textbf{方法贡献}：提出DeepLeakyU-Net网络架构，通过在编码器中全面采用Leaky ReLU激活函数和使用步长卷积替代池化层，改善深层编码器的梯度流动和空间信息保留，提升重建质量。
